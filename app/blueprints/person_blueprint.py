...
def actors_list():
    ...
    results = models.Person.query.join(
        models.Person, models.Show.cast).paginate(page, per_page)
    return jsonify(
        {
...
def actors_detail(actor_id: int):
    actor = models.Person.query.join(models.Show, models.Person.cast).filter(
        models.Person.id == actor_id).first()
...
def director_list():
    ...
    results = models.Person.query.join(
        models.Person, models.Show.directors).paginate(page, per_page)
    return jsonify(
        {
            'result': persons_schema.dump(results.items),
            'page': page,
...
def director_details(actor_id: int):
    director = models.Person.query.join(models.Person, models.Show.directors).filter(
        models.Person.id == actor_id).first()
